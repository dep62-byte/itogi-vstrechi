...
Google-запись выполняется только после Drive.about: dep6.2. Секреты остаются в штатном credential store. Оригиналы кейсов включены в открытый пакет с разрешения владельца и проверяются SHA256. Текстовые кейсы явно отделены от кейсов с изображением. Весь архив Telegram, личные Downloads и сырьё продавцам не нужны.
